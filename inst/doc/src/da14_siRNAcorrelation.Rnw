...
$\pi$-scores are summarized over two replicates. Pearson correlation coefficients are computed for interaction profiles for each pair of siRNA and saved in the matric $C$.
<<siRNAcorrelation2>>=
