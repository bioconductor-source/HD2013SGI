...
$\pi$-scores are normalized per feature by dividing with the median
deviation between two replicates. For each siRNA-pair they are
...
PI = (PI[,,,,,1]+PI[,,,,,2]) / 2
@ 
The 5-dimensional array is reshaped to a 3-dimensional array by
flatening the dimensions of target genes and siRNA designs as well as
...
@
The dimension of the new matrix is \Sexpr{dim(PI)[1]} target siRNA
$\times$ \Sexpr{dim(PI)[2]} query siRNA
$\times$ \Sexpr{dim(PI)[3]} phenotypic features. The color scale for
interaction scores is cutted from above and below by
$\pm$6. Interaction scores between -2 and 2 are colored black. 
<<heatmap4>>=
cuts = c(-Inf,
...
$\pi$-scores are normalized per feature by dividing with the median
deviation between two replicates. For each gene pair $\pi$-scores are
