...
Main effects are first summarized over all four siRNA design pairs and
divided by the median deviation to compute a z-score. Bars show the
...
Interactions with an adjusted p-value of 0.01 are considered. 
...
SIG[is.na(SIG)] = FALSE
@ 
The 5-dimensional array of boolean values indicating the significant
interactions is reshaped to a 2-dimensional table with one column for
each feature.
...
@
A contingency table is computed considering all gene pairs that interact in
...
A Venn diagram can be generated with a webtool
(e.g. http://bioinfogp.cnb.csic.es/tools/venny/index.html). Lists of
identifiers of the significant genes are written be be used with the webtools.
...
In the original paper a Venn diagram is shown. An alternative plot
...
@ 
<<overlapOfInteractions7,results=hide>>=
dev.off()
@
\begin{center}
