...
phenotypic feature is counted. Only significant interactions (FDR <=
0.01) are considered.
...
are sorted by the total number (pos. and neg.) interactions starting
...
nneg = nneg[I]
@ 
A barchart is plotted showing the number of positive and negative interactions.
