...
dir.create(file.path("result","Figures","additional"),recursive=TRUE,showWarnings=FALSE)
...
<<featureSelection7>>=
save(stabilitySelection, file=file.path("result","data","stabilitySelection.rda"))
...
pdf(file.path("result","Figures","additional","callingInteractions.pdf"),height=10)
D = pimatrix$D
...
for (i in 1:dim(D)[5]) {
  ...
  par(mfrow=c(2,1))
  plot(Data,pch=20,cex=0.4,main=dimnames(D)[[5]][i])
  ...
  PADJ[,,,,i] = padj
  plot(Data,pch=20,col=ifelse(padj < 0.03,"red","black"),
       cex=0.4,main="with filtering")
