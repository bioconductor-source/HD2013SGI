...
Each plate contains the same set of target siRNAs. In all wells of one plate the same query siRNA is pipetted. The query siRNA on the \Sexpr{length(platenr)} plates are grouped in sample (double gene knock down) and negative control to measure the main effects (single knock down effects) of the target genes.
<<convertData3>>=
